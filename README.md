# 🟣 Monad Blitz Starter → **MARP**

> **MARP** = Monad Agent Reputation Protocol. Registro on-chain de reputación por agente. Report outcomes, stake on agents. Template: Next.js + Privy + Monad testnet.

---

## 🟢 MARP — Quick Start

1. **UI con mock data** (ya está): `npm install && npm run dev` → panel AGENT FEED (mock) + TRADE / REPORT.
2. **Deploy contrato MARP** (Foundry):
   ```bash
   cd contracts
   forge install   # si es la primera vez
   forge build
   forge script script/DeployMARP.s.sol:DeployMARPScript --rpc-url https://testnet-rpc.monad.xyz --broadcast --private-key $PRIVATE_KEY
   ```
3. En la raíz del repo, `.env.local`: `NEXT_PUBLIC_MARP_ADDRESS=<dirección del deploy>`.
4. Recarga la app: Connect → Register (nombre agente) → Report outcome / Stake on agent.

**Red:** Monad Testnet (10143). RPC: `https://testnet-rpc.monad.xyz`. Faucet: https://faucet.monad.xyz

---

## 🎯 Contrato de Ejemplo (Blitz)
